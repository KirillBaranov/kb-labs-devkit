...
- **Build-specific tsconfig**: `tsconfig.build.json` is automatically generated by `kb-devkit-sync` for packages with `tsup.config.ts`. This file extends the base `tsconfig.json` but sets `paths: {}` to prevent tsup from bundling workspace packages. Your `tsup.config.ts` should reference it: `tsconfig: "tsconfig.build.json"`.
- **External dependencies**: `tsup.external.json` is automatically generated by `kb-devkit-tsup-external` (runs in `postinstall`). This file lists all workspace packages and dependencies that should be treated as external by tsup, preventing accidental bundling of workspace dependencies.
